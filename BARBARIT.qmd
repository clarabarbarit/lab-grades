...
dragon_grades <- grades |>
  ...
  left_join(students, by = "id") |>
  select(id, group, `number of grades in Dragon Lore and Taming`, sex)
...
```{r, message=FALSE, warning=FALSE}
dragon_grades |>
  ggplot(aes(x = group, y = `number of grades in Dragon Lore and Taming`)) +
  geom_violin() +
  labs(title = "Dragon Lore Grades per Student by Group")+
  theme_minimal()
```

```{r, message=FALSE, warning=FALSE}
dragon_grades |>
  ggplot(aes(x = group, y = `number of grades in Dragon Lore and Taming`, fill = sex)) +
  geom_violin() +
  labs(title = "Dragon Lore Grades by Group and Sex")+
  theme_minimal()
```

```{r, message=FALSE, warning=FALSE}
avg_course_long <- grades |>
  filter(!is.na(grade)) |>
  group_by(id, course_id) |>
  summarise(avg_grade = round(mean(grade), 2), .groups = "drop") |>
  left_join(courses, by = "course_id") |>
  select(id, course, avg_grade)

avg_course <- avg_course_long |>
  pivot_wider(
    names_from = course,
...
avg_course <- students |>
  select(id, any_of("group"), everything()) |>
  right_join(avg_course, by = "id")
...
avg_course |>
  ggplot(aes(
    x = `Ancient Magic and Mysticism`,
    y = `Illusion and Enchantment`
  )) +
  geom_point(alpha = 0.7) +
  geom_smooth(se = FALSE) +
  labs(title = "Relationship Between Ancient Magic and Illusion Grades")
```

```{r, message=FALSE, warning=FALSE}
avg_group <- avg_course |>
  select(-group) |>
  left_join(students |> select(id, group), by = "id") |>
  group_by(group) |>
  summarise(
    corr = cor(`Necromancy and Spirit Summoning`,
...
avg_course |>
  ...
  ggplot(aes(
    x = `Illusion and Enchantment`,
    y = `Necromancy and Spirit Summoning`
  )) +
  geom_point() +
  geom_smooth(se = FALSE) +
  labs(title = paste("Relationship in Group", best_group))
```

```{r, message=FALSE, warning=FALSE}
final_grades <- avg_course |>
  mutate(
    final = rowMeans(across(where(is.numeric) & !c(id) & !c(group)), na.rm = TRUE)
  ) |>
  select(id, group, sex, final) |>
  arrange(desc(final))
...
final_grades |>
  ...
  geom_violin() +
  labs(title = "Final Grades by Group")+
  theme_minimal()
```

```{r, message=FALSE, warning=FALSE}
final_grades |>
  ggplot(aes(x = group, y = final, fill = sex)) +
  geom_violin() +
  labs(title = "Final Grades by Group and Sex")+
  theme_minimal()
```
...
passes <- avg_course |>
  pivot_longer(
    cols = where(is.numeric) & !c(id) & !c(group),
    names_to = "course",
    values_to = "avg_by_course"
  ) |>
  left_join(courses |> select(course, module), by = "course") |>
  group_by(id, group, sex) |>
  summarise(
    min_course_avg = min(avg_by_course, na.rm = TRUE),
    module1 = mean(avg_by_course[module == 1], na.rm = TRUE),
    module2 = mean(avg_by_course[module == 2], na.rm = TRUE),
    pass = (min_course_avg >= 5 &
            module1 >= 10 &
            module2 >= 10),
    .groups = "drop"
  )
...
```{r, message=FALSE, warning=FALSE}
passes |>
  left_join(final_grades, by = "id") |>
  filter(!pass & final >= 10) |>
  nrow()
```
...
The course that is the most responsible of student failure owing to grades below than 5 is Runecrafting and Glyphwork.
...
passes |>
  ...
  geom_line() +
  geom_point() +
  labs(title = "Success Rate as a Function of Age")+
  theme_minimal()
```
